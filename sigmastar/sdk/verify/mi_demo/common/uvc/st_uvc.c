    ...
    /* get dwc3 current_speed: usb3.0:super-speed usb2.0:high-speed */
      ...
      if(!access("/sys/class/udc/1f344200.dwc3/current_speed", 0))
     {
        ...
        ST_UVC_GetCurrentSpeed("cat /sys/class/udc/1f344200.dwc3/current_speed", speedName, sizeof(speedName));
         ...
         {
            bSuperSpeed = true;
            printf("This is dwc3 usb3.0 current_speed=%s\n", speedName);
         }
         else if(0 == strncmp("high-speed", speedName, 10))
         {
            bSuperSpeed = false;  
            printf("This is dwc3 usb2.0 current_speed=%s\n", speedName);
         }
         bDwc3 = true;
     }
     else if(!access("/sys/class/udc/soc\:Sstar-udc/current_speed", 0))
     {
        ...
        {
            printf("This is udc-msb250x usb2.0 current_speed=%s\n", speedName);
        }
        bDwc3 = false;
    }
        ...
        */
        ...
        if(bDwc3)
        {
               ...
               switch (format->fcc) {
                case V4L2_PIX_FMT_YUYV:
                    if(frame->width*frame->height == 3840*2160)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 2.0) * 15 / 8000;
                    break;
                case V4L2_PIX_FMT_NV12:
                    if(frame->width*frame->height == 3840*2160)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 1.5) * 15 / 8000;
                    break;
                case V4L2_PIX_FMT_MJPEG:
                    if(!strcmp(pdev->name, "/dev/video2"))
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 20 / 8000;
                    break;
                case V4L2_PIX_FMT_H264:
                    if(frame->width*frame->height > 1920*1080)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 25 / 8000;
                    break;
                case V4L2_PIX_FMT_H265:
                   if(frame->width*frame->height > 1920*1080)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 25 / 8000;
            ...
            else
            {
                ...
                case V4L2_PIX_FMT_YUYV:
                    if(frame->width*frame->height > 1920*1080)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 2.0) * 1 / 8000;
                    else if(frame->width*frame->height > 1280*720)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 2.0)  * 5 / 8000;
                    else if(frame->width*frame->height > 960*540)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 2.0)  * 10 / 8000;
                    break;
                case V4L2_PIX_FMT_NV12:
                   if(frame->width*frame->height > 1920*1080)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 1.5) * 1 / 8000;
                    else if(frame->width*frame->height > 1280*720)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 1.5) * 5 / 8000;
                    else if(frame->width*frame->height > 960*540)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height * 1.5) * 10 / 8000;
                    break;
                case V4L2_PIX_FMT_MJPEG:
                     if(frame->width*frame->height > 1920*1080)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 2.5 / 8000;
                    else if(frame->width*frame->height > 1280*720)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 8 / 8000;
                    else if(frame->width*frame->height > 960*540)
                        ctrl.dwMaxPayloadTransferSize = (frame->width*frame->height*2.0)/2 * 25 / 8000;
                    break;
                case V4L2_PIX_FMT_H264:
                ...
                if(!strcmp(pdev->name, "/dev/video2"))
                        ctrl.dwMaxPayloadTransferSize = ctrl.dwMaxVideoFrameSize * 20 / 8000;
            }
            if(FrameInterval2FrameRate(ctrl.dwFrameInterval) > 30)
            {
                /** default MaxPayloadTransferSize **/
                ctrl.dwMaxPayloadTransferSize = (ctrl.dwMaxVideoFrameSize * FrameInterval2FrameRate(ctrl.dwFrameInterval) / 8000);
            }
        ...
        {   /* udc-msb250x usb2.0  */
            ctrl.dwMaxPayloadTransferSize = (MaxPayloadTransferSize <= 3072) ? MaxPayloadTransferSize:3072;
        }
        printf("dwMaxPayloadTransferSize=%d and Frame=%dfps\n", ctrl.dwMaxPayloadTransferSize, FrameInterval2FrameRate(ctrl.dwFrameInterval));
    }
